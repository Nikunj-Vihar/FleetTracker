...
import { listVehicleAuditLogs } from "@/lib/store";
import type { Vehicle, VehicleAuditLogRecord } from "@/lib/types";
...
export default function VehicleAuditTrailModal({ vehicle, onClose }: { vehicle: Vehicle; onClose: () => void }) {
  const [logs, setLogs] = useState<VehicleAuditLogRecord[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    listVehicleAuditLogs(vehicle.id).then((l) => {
      setLogs(l);
      setLoading(false);
    });
  }, [vehicle.id]);
      ...
      <div className="glass-panel-solid max-h-[80vh] w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Audit Trail — {vehicle.vehicle_no}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="max-h-[65vh] overflow-y-auto p-4">
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-slate-400">
              No corrections yet — this vehicle is exactly as originally added.
            </p>
          ) : (
