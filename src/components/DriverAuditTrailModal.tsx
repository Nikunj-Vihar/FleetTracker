...
import { listDriverAuditLogs } from "@/lib/store";
import type { Driver, DriverAuditLogRecord } from "@/lib/types";
...
export default function DriverAuditTrailModal({ driver, onClose }: { driver: Driver; onClose: () => void }) {
  const [logs, setLogs] = useState<DriverAuditLogRecord[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    listDriverAuditLogs(driver.id).then((l) => {
      setLogs(l);
      setLoading(false);
    });
  }, [driver.id]);
      ...
      <div className="glass-panel-solid max-h-[80vh] w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Audit Trail — {driver.name}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="max-h-[65vh] overflow-y-auto p-4">
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-slate-400">
              No corrections yet — this driver is exactly as originally added.
            </p>
          ) : (
