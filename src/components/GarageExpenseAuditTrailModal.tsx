...
import { listGarageExpenseAuditLogs } from "@/lib/store";
import type { GarageExpense, GarageExpenseAuditLogRecord } from "@/lib/types";
...
export default function GarageExpenseAuditTrailModal({
  expense,
  onClose,
}: {
  ...
  const [logs, setLogs] = useState<GarageExpenseAuditLogRecord[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    listGarageExpenseAuditLogs(expense.id).then((l) => {
      setLogs(l);
      setLoading(false);
    });
  }, [expense.id]);
